...
class PUFDataset(Dataset):
    def __init__(self, folder, ids, transform):
        ...
        self.denormalize = lambda response: ((response + 1) * (max - min) / 2) + min
    ...
    def __getitem__(self, idx):
        ...
        response = self.transform(response).float() if self.transform is not None else response.float()
...
class PUFDataModule(LightningDataModule):
    def __init__(self, batch_size, folder, training_ids, test_ids):
        ...
        self.folder = folder
        self.train_kwargs = {"batch_size": self.batch_size, "num_workers": 8, "pin_memory": True, "shuffle": True}
        self.val_test_kwargs = {"batch_size": self.batch_size, "num_workers": 8, "pin_memory": True, "shuffle": True}
    ...
    def setup(self):
        transform = transforms.Compose([
            CenterCrop512(),
            transforms.ToTensor()
        ]) if "Real" in self.folder else None
        self.train_dataset = PUFDataset(self.folder, self.training_ids, transform)
        self.test_dataset = PUFDataset(self.folder, self.test_ids, transform)
